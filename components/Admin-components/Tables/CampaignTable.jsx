...
export default function CampaignTable() {
  ...
  function showModal(mode, campaign_id = null) {
    ...
    if (mode == "Edit") {
      const filteredCampaign = campaigns.filter(
        (campaign) => campaign._id === campaign_id
      );
      if (filteredCampaign.length > 0) {
        setValues({
          name: filteredCampaign[0].name,
          start_date: filteredCampaign[0].start_date,
          end_date: filteredCampaign[0].end_date,
          account: filteredCampaign[0].account_id,
          objective: filteredCampaign[0].objective,
          status: filteredCampaign[0].status,
        });
        setError({
          name: "",
          start_date: "",
        ...
        if (userData.roles == "sadmin") {
          document.getElementById("tenant").value =
            filteredCampaign[0].tenant_id;
          document.getElementById("tenant").disabled = true;
        }
        handleGetAccountList(filteredCampaign[0].tenant_id);
        document.getElementById("account").value =
          filteredCampaign[0].account_id;
        document.getElementById("objective").value =
          filteredCampaign[0].objective;
        document.getElementById("start_date").value = dateconvert(
          filteredCampaign[0].start_date
        );
        filteredCampaign[0].end_date
          ? (document.getElementById("end_date").value = dateconvert(
              filteredCampaign[0].end_date
            ))
          : filteredCampaign[0].end_date
          ? (document.getElementById("end_date").value = dateconvert(
  ...
  const deleteCampaign = async (campaing_id) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/campaign-delete?campaign_id=${campaing_id}`,
        {
          headers: {
  ...
  async function getCampaign() {
    ...
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/campaign-by-tenant`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
        },
  ...
  function getCampaignNoAsync() {
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/campaign-by-tenant`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
        },
      })
      .then((response) => {
        ...
        setTimeout(() => {
          ...
          createMetrics(
            lastData._id,
            dateconvert(lastData.start_date),
            dateconvert(lastData.end_date)
          );
        }, 5000); // Menunda selama 5 detik
    ...
    if (EditCampaignId !== null) {
      if (isvalid) {
        ...
        const response = await axios.put(
          `${process.env.NEXT_PUBLIC_API_URL}/campaign-edit?campaign_id=${EditCampaignId}`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
  ...
  async function getSelectFrontend() {
    ...
    await axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/timezone`)
      .then((response) => {
        setTimezone(response.data);
      });

    await axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/currency`)
      .then((response) => {
        setCurrency(response.data);
      });

    await axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/culture`)
      .then((response) => {
        setCulture(response.data);
      });

    await axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/account-by-tenant`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
        },
      })
    ...
    try {
      const satu = axios
        .post(
          ...
          }/metrics-create?campaign_id=${campaignID}&tenantId=${localStorage.getItem(
            "tenantId"
          )}`,
          formDataMetrics,
          {
        ...
        .then((response) => {
          if (!response.IsError) {
            for (let i = 0; i < selisihHari; i++) {
              ...
              const reach2 = parseInt(
                getRandomFloat(amountspent2 / 10, amountspent2)
              );
              ...
              try {
                const dua = axios.put(
                  `${process.env.NEXT_PUBLIC_API_URL}/metrics-hitung?campaign_id=${campaignID}`,
                  formDua
                );
              } catch (error) {
    ...
    <>
      <div className="w-full">
        <div className="flex flex-col md:flex-row justify-between items-center mb-10">
          <h1 className="text-3xl font-bold dark:text-white uppercase flex gap-2">
            {" "}
            <RiMegaphoneLine size={35} /> {t("title")}
          </h1>
            ...
            <div className="flex flex-col-reverse md:flex-row justify-between items-center w-full ">
              <div className="w-full flex justify-start items-center flex-col sm:flex-col md:flex-col lg:flex-row xl:flex-row">
                <div className="flex mb-4 lg:mb-0 xl:mb-0">
                  ...
                  <button
                    title="Export Pdf"
                    className="h-10 bg-white dark:bg-slate-800 py-2 sm:py-2 md:py-2 lg:mb-4 xl:mb-4 border dark:border-gray-500 hover:bg-gray-100 dark:hover:bg-slate-500 font-bold px-3 rounded-s-md"
                    onClick={generatePDF}
                  >
                  ...
                  <button
                    title="Export Excel"
                    className="h-10 bg-white dark:bg-slate-800 py-2 sm:py-2 md:py-2 lg:mb-4 xl:mb-4 border-b border-t border-e dark:border-gray-500 hover:bg-gray-100 dark:hover:bg-slate-500 font-bold px-3"
                  ...
                  <button
                    title="Refresh"
                    className="h-10 bg-white dark:bg-slate-800 py-2 sm:py-2 md:py-2 lg:mb-4 xl:mb-4 border-b border-t dark:border-gray-500 hover:bg-gray-100 dark:hover:bg-slate-500 font-bold px-3"
                  ...
                  <button
                    title="Add Data"
                    className="h-10 bg-white dark:bg-slate-800 py-2 sm:py-2 md:py-2 lg:mb-4 xl:mb-4 border dark:border-gray-500 hover:bg-gray-100 rounded-e-md sm:rounded-e-md md:rounded-e-md lg:rounded-e-none xl:rounded-e-none dark:hover:bg-slate-500 font-bold px-3 "
                  ...
                  {userData.roles == "sadmin" && (
                    <div>
                      ...
                      <select
                        id="tenantfilter"
                        className="md:w-[150px] h-10 bg-white dark:bg-slate-800 border-b border-t border-e dark:border-gray-500 text-sm rounded-e-md md:rounded-none block w-full px-3 py-2 select-no-arrow"
                ...
                <div className="flex">
                  ...
                  <div className="mb-4">
                    ...
                    <select
                      id="rolefilter"
                      className="md:w-[150px] h-10 bg-white dark:bg-slate-800 border-t border-b border-s dark:border-gray-500 sm:border-s md:border-s lg:border-s-0 xl:border-s-0 rounded-s-md sm:rounded-s-md md:rounded-s-md lg:rounded-s-none xl:rounded-s-none text-sm block w-full px-3 py-2 select-no-arrow"
                  ...
                  <div className="mb-4">
                    ...
                    <select
                      id="rolefilter"
                      className="md:w-[150px] h-10 bg-white dark:bg-slate-800 border dark:border-gray-500 text-sm block w-full px-3 py-2 select-no-arrow"
                  ...
                  <div className="mb-4">
                    ...
                    <select
                      id="objectivefilter"
                      className="md:w-[150px] h-10 bg-white dark:bg-slate-800 border-b border-t border-e dark:border-gray-500 text-sm rounded-e-md block w-full px-3 py-2 select-no-arrow"
              ...
              <div className="flex gap-5">
                <div className="relative mb-4 ">
                  ...
                  <input
                    ...
                    id="search"
                    name="search"
                    className="h-10 w-full px-4 py-2 border dark:border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-slate-800"
                ...
                <tbody className="bg-white dark:bg-slate-800">
                  {isLoading ? (
                    ...
                    currentcampaigns.map((campaign, index) => (
                      <tr
                        key={index}
                        className="hover:bg-gray-100 dark:hover:bg-slate-400 dark:odd:bg-slate-600 dark:even:bg-slate-700 hover:cursor-pointer transition-colors duration-300"
                        onClick={() => showModal("Edit", campaign._id)}
                      >
                        <td
                          scope="row"
                          className="px-5 py-3 border dark:border-gray-500 font-medium whitespace-nowrap"
                        >
                          {campaign.name}
                        </td>
                        <td
      ...
      >
        <div
          ...
          onClick={closeModal}
        ></div>
        <div className="relative z-40 mt-1 w-screen md:w-full max-w-2xl max-h-screen">
          {/* <!-- Modal content --> */}
            ...
            <div className="p-4 md:p-5">
              ...
              >
                <div className="grid gap-4 mb-4 grid-cols-2 bg-white dark:bg-[#243040] dark:text-white overflow-y-auto mt-[4.5rem]">
                  <div
                    className={` ${
                      userData.roles == "sadmin" ? "col-span-2" : "col-span-1"
                    }`}
                  >
                    <label
                      htmlFor="name"
                      className="mb-2 text-sm font-normal text-black dark:text-slate-200  flex"
                    >
                      {t("campaign_name")}{" "}
                      <div className="text-red-500 dark:text-red-600">*</div>
                    </label>
                    <input
                      type="text"
                      name="name"
                      id="name"
                      required
                      className="bg-white border dark:bg-[#1d2a3a] dark:border-[#314051] border-gray-200 placeholder-[#858c96]  text-sm rounded-[3px] focus:ring-[#3c54d9] focus:border-[#3c54d9] outline-none block w-full p-2.5 text-black dark:text-slate-200"
                      placeholder={t("holder-name")}
                      onChange={handleChange}
                      onBlur={handleBlur}
                    />
                    {touched.name && error.name ? (
                      <div className="text-red-500 dark:text-red-600 text-sm">
                        {error.name}
                      </div>
                    ) : (
                      ""
                  ...
                  {userData.roles == "sadmin" && (
                    <div className="col-span-1" ref={tenantInput}>
                      <label
                        htmlFor="tenant"
                        className="block mb-2 text-sm font-normal text-black dark:text-slate-200 "
                      >
                        Tenant
                      </label>
                      <select
                        id="tenant"
                        onChange={(e) => handleGetAccountList(e.target.value)}
                        required
                        defaultValue={""}
                        name="tenant"
                        className="bg-white border text-[#858c96] dark:bg-[#1d2a3a] dark:border-[#314051] border-gray-200 placeholder-[#858c96]  text-sm rounded-[3px] focus:ring-[#3c54d9] focus:border-[#3c54d9] outline-none block w-full p-2.5  "
                      >
                  ...
                  <div className="col-span-1">
                    <label
                      htmlFor="account"
                      className="flex mb-2 text-sm font-normal text-black dark:text-slate-200"
                    >
                      {t("account")}{" "}
                      <div className="text-red-500 dark:text-red-600">*</div>
                    </label>
                    <select
                      ...
                      name="account"
                      required
                      className={`bg-white border dark:bg-[#1d2a3a] dark:border-[#314051] ${
                        values.account
                          ? "text-black dark:text-white"
                          : "text-[#858c96]"
                      } border-gray-200 placeholder-[#858c96]  text-sm rounded-[3px] focus:ring-[#3c54d9] focus:border-[#3c54d9] outline-none block w-full p-2.5`}
                      defaultValue={""}
                      onChange={handleChange}
                      onBlur={handleBlur}
                    >
                  ...
                  <div className="col-span-1">
                    ...
                    <select
                      ...
                      className={`bg-white ${
                        values.objective
                          ? "text-black dark:text-white"
                          : "text-[#858c96]"
                      } border dark:bg-[#1d2a3a] dark:border-[#314051] border-gray-200 placeholder-[#858c96]  text-sm rounded-[3px] focus:ring-[#3c54d9] focus:border-[#3c54d9] outline-none block w-full p-2.5`}
                      defaultValue={""}
                      onChange={handleChange}
                      onBlur={handleBlur}
                    >
                    ...
                    {touched.objective && error.objective ? (
                      <div className="text-red-500 dark:text-red-600 text-sm">
                        {error.objective}
                      </div>
                    ) : (
                    ...
                    <input
                      ...
                      className={`bg-white border ${
                        values.start_date
                          ...
                          : "text-[#858c96]"
                      } dark:bg-[#1d2a3a] dark:border-[#314051] border-gray-200 placeholder-[#858c96]  text-sm rounded-[3px] focus:ring-[#3c54d9] focus:border-[#3c54d9] outline-none block w-full p-2.5`}
                      placeholder="Type Campaign name here"
                      onChange={handleChange}
                      onBlur={handleBlur}
                    />
                    {touched.start_date && error.start_date ? (
                      <div className="text-red-500 dark:text-red-600 text-sm">
                        {error.start_date}
                      </div>
                    ) : (
                  ...
                  <div className="col-span-1">
                    ...
                    <input
                      ...
                      onBlur={handleBlur}
                    />
                    {touched.end_date && error.end_date ? (
                      <div className="text-red-500 dark:text-red-600 text-sm">
                        {error.end_date}
                      </div>
                    ) : (
                  ...
                  <div className="col-span-2">
                    <label
                      htmlFor="notes"
                      className="mb-2 text-sm font-normal text-black dark:text-slate-200"
                    >
                      Notes
                    </label>
                    <textarea
                      id="notes"
                      name="notes"
                      className="bg-white border text-black dark:text-slate-200 dark:bg-[#1d2a3a] dark:border-[#314051] border-gray-200 placeholder-[#858c96] text-sm rounded-[3px] focus:ring-[#3c54d9] focus:border-[#3c54d9] outline-none block w-full p-2.5"
                ...
                {modeModal === "Edit" ? (
                  <div className="flex gap-3">
                    ...
                    <div
                      ...
                      onClick={() => handleDelete(EditCampaignId)}
                    >
                      {t("delete")}
                    </div>
                  </div>
