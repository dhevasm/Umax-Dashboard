...
export default function UserTable() {
  ...
  const deleteuser = async (user_id) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/user-delete?user_id=${user_id}`,
        {
          headers: {
  ...
  async function getUsers() {
    ...
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/user-by-tenant`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
        },
  ...
  async function updateUser() {
    if (EditUserId !== null) {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/change-user-role?user_id=${EditUserId}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
      ...
      if (
        response.data.Output.includes("Successfully changed") ||
        response.data.Output.includes("Berhasil")
      ) {
        getUsers();
        closeModal();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const url =
        roles == "sadmin"
          ? `${process.env.NEXT_PUBLIC_API_URL}/register?tenantId=${values.tenant_id}`
  ...
  const filteredData = users.filter((data) => {
    ...
    return (
      ...
      (!selectedTenant ||
        data.tenant_id.toLowerCase() == selectedTenant.toLowerCase()) &&
      (!searchTerm ||
    ...
    <>
      <div className="w-full dark:text-white">
        <div className="flex flex-col md:flex-row justify-between items-center mb-10">
          <h1 className="text-3xl font-bold flex dark:text-white gap-2">
            {" "}
            <RiUser3Line size={35} /> {t("title")}
          </h1>
          <p className="dark:text-white">
            <span
              className="hover:cursor-pointer hover:text-blue-400 dark:text-white hover:underline"
            ...
            <div className=" flex flex-col-reverse md:flex-row justify-between items-center w-full ">
              ...
              >
                <div className="flex mb-4">
                  ...
                  <button
                    title="Export Pdf"
                    className="h-10 bg-white dark:border-gray-500 dark:bg-slate-800 py-2 border hover:bg-gray-100 dark:hover:bg-slate-400 font-bold px-3 rounded-s-md"
                    onClick={generatePDF}
                  >
                  ...
                  <button
                    title="Export Excel"
                    className="bg-white dark:border-gray-500 dark:bg-slate-800 py-2 border-b border-t border-e hover:bg-gray-100 dark:hover:bg-slate-400 font-bold px-3"
                  ...
                  <button
                    title="Add Data"
                    className={`bg-white dark:border-gray-500 dark:bg-slate-800 py-2 border-b border-t border-e ${
                      userData.roles == "sadmin"
                        ? "rounded-e-md md:rounded-e-none"
                        : ""
                    } hover:bg-gray-100 dark:hover:bg-slate-400 font-bold px-3 `}
                    onClick={createModal}
                  >
                    <IconContext.Provider value={{ className: "text-xl" }}>
                      <BiPlus />
                ...
                <div className="mb-4 flex">
                  {userData.roles == "sadmin" && (
                    <>
                      ...
                      <select
                        id="tenantfilter"
                        className="md:w-[150px] h-10 dark:border-gray-500 bg-white dark:bg-slate-800 border-s border-b  rounded-s-md md:rounded-s-none border-t border-e text-sm block w-full px-3 py-2"
                        defaultValue={0}
                      ...
                      >
                        ...
                        {tenants.map((tenant, index) => {
                          return (
                            <option value={tenant._id} key={index + 1}>
                              {tenant.company}
                            </option>
                          );
                        })}
                  ...
                  <select
                    id="rolefilter"
                    className="md:w-[150px] dark:border-gray-500 h-10 bg-white dark:bg-slate-800 border-b border-t border-e rounded-e-md text-sm block w-full px-3 py-2"
                    defaultValue={0}
                    value={selectedRole}
              ...
              <div className="flex gap-5">
                <div className="relative mb-4">
                  <input
                    type="text"
                    className="w-full h-10 dark:bg-slate-800 px-4 py-2 border dark:border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              ...
              <table className="w-full text-sm text-left">
                <thead className="text-md text-left uppercase bg-white dark:bg-slate-700">
                  <tr>
                    <th
                      scope="col"
                      className="px-6 border dark:border-gray-500 py-3"
                    >
                      {t("name")}
                    </th>
                    <th
                ...
                <tbody className="bg-white dark:bg-slate-800">
                  {isLoading ? (
                    ...
                    currentusers.map((user, index) => (
                      ...
                      >
                        <td
                          className="px-6 border dark:border-gray-500 py-3 font-medium whitespace-nowrap underline"
                          title="Click to edit"
            ...
            <div className="p-4 md:p-5">
              <div className="grid gap-4 mb-4 grid-cols-2">
                <div className="col-span-2">
                  ...
                  <input
                    ...
                    name="name"
                    id="name"
                    className="bg-white dark:bg-[#1d2a3a] text-black dark:text-slate-200 placeholder-[#858c96] border dark:border-[#314051] border-gray-200  text-sm rounded-[3px] focus:ring-[#3c54d9] focus:border-[#3c54d9] outline-none block w-full p-2.5 "
                ...
                <div
                  className={`col-span-2 ${
                    EditUserId == "64fa84403ce06f0129321ced" ? "hidden" : ""
                  }`}
                >
                  <label
                    htmlFor="role"
                  ...
                  <select
                    id="role"
                    className="bg-white border dark:border-[#314051] border-gray-200 text-black dark:text-slate-200 dark:bg-[#1d2a3a] placeholder-[#858c96]  text-sm rounded-[3px] focus:ring-[#3c54d9] focus:border-[#3c54d9] outline-none block w-full p-2.5 "
                  >
              ...
              <div className="flex flex-row-reverse items-center gap-3 mt-5">
                {EditUserId != "64fa84403ce06f0129321ced" ? (
                  <button
                    className="w-full bg-indigo-700 hover:bg-indigo-600 border border-indigo-800 text-white py-2 px-4 rounded text-nowrap"
                ...
                {modeModal === "Edit" ? (
                  <button
                    className="w-full bg-[#3b50df] hover:bg-blue-600 border border-indigo-700 text-white py-2 px-4 rounded text-nowrap disabled:cursor-progress"
      ...
      <div
        className={`fixed inset-0 ${
          create ? "flex" : "hidden"
        } items-center justify-center bg-gray-500 bg-opacity-60 z-50`}
      >
        <div
          className="w-screen h-screen fixed top-0 left-0"
          onClick={createModal}
              ...
              <form onSubmit={formik.handleSubmit}>
                <div className="grid gap-4 mb-4 grid-cols-2">
                  <div className="col-span-2 md:col-span-1">
                    <label
                      htmlFor="username"
                      className="block mb-1 text-sm font-normal text-black dark:text-slate-200"
                    >
                      {t("name")}
                    </label>
                    <input
                ...
                <div className="flex justify-end gap-3 mt-5">
                  <button
                    type="submit"
                    disabled={crudLoading}
                    className="w-full bg-indigo-700 hover:bg-indigo-600 border border-indigo-800 text-white font-bold py-2 px-4 rounded disabled:cursor-progress"
                  >
                    {crudLoading ? <LoadingCrud /> : t("submit")}
                  </button>
                </div>
