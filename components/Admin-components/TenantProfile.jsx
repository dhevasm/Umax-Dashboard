...
export default function TenantProfile({ tenant_id }) {
  ...
  async function getTenant() {
    const response = await axios
      .get(
        `${process.env.NEXT_PUBLIC_API_URL}/tenant-by-id?tenant_id=${tenant_id}`,
        {
          headers: {
  ...
  function showModal(mode, tenant_id = null) {
    ...
    if (mode == "Edit") {
      if (tenant !== null) {
        ...
        if (tenant.currency_position == true) {
          document.getElementById("currencyposition-left").checked = true;
        } else {
          document.getElementById("currencyposition-right").checked = true;
        }
  ...
  async function updateTenant() {
    ...
    if (tenant !== null) {
      ...
      if (isvalid) {
        ...
        const currencyposition = document.getElementById(
        ...
        const response = await axios.put(
          `https://umaxxnew-1-d6861606.deta.app/tenant-edit?tenantId=${tenant._id}`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
    ...
    <>
      <div className="w-full h-full rounded-sm">
        <div className="flex flex-col sm:flex-row md:flex-row lg:flex-row xl:flex-row justify-between items-center">
          <h1 className="text-2xl dark:text-white font-bold uppercase">
            {t("title")}
          </h1>
          <div className="dark:text-white">
            <a
              className="hover:cursor-pointer hover:text-blue-400 hover:underline dark:text-white"
        ...
        <div className="rounded-sm mt-3 rounded-t-md">
          <div className="w-full flex justify-between h-[30vh] bg-bg-tenant gap-4 items-end px-5 py-5 rounded-t-md">
            {tenant.company ? (
              <h1 className="font-bold text-2xl text-white drop-shadow-xl">
                {tenant.company}
              </h1>
            ) : (
            ...
            {tenant.company ? (
              <div
                className="self-end text-xs md:text-md text-nowrap flex gap-2 items-center text-white hover:cursor-pointer dark:hover:bg-slate-700 dark:bg-slate-700 bg-[#3d50e0] hover:bg-blue-600 p-2 rounded-md"
                onClick={() => showModal("Edit", tenant._id)}
              >
                <FaPen />
                {t("edit-profile")}
              </div>
              ...
              <>
                ...
                <div className="flex flex-row gap-4 items-center mt-3 p-5 bg-gray-50 dark:bg-slate-700 rounded-sm outline-none">
                  ...
                  <div className="ml-2 dark:text-white">
                    ...
                    <a
                      href={`https://wa.me/${tenant.contact.replace(
                        /\D+/g,
                        ""
                      )}`}
                      target="_blank"
                ...
                <div className="grid grid-cols-1 md-grid-cols-2 lg:grid-cols-2 xl:grid-cols-2    justify-between gap-5 mt-5 pb-5 bg-white dark:bg-gray-800 rounded-sm outline-none">
                  <div className="flex gap-4 items-center p-5 bg-gray-50 dark:bg-slate-700 rounded-sm outline-none w-full">
                    ...
                    <p className="ml-2 dark:text-white">
                      <span className="font-semibold">{t("culture")}</span>
                      <br /> {getCountryNameFromCode(tenant.culture)}
                    </p>
                  </div>
                  <div className="flex gap-4 items-center p-5 bg-gray-50 dark:bg-slate-700 rounded-sm outline-none w-full">
                    ...
                    <p className="ml-2 dark:text-white">
                      ...
                      <span className="flex gap-2 items-center">
                        <Image
                          src={getFlagSrc(tenant.language)}
                          width={50}
                          height={50}
                          className="h-5 w-5"
                          alt="flag"
                  ...
                  <div className="flex gap-4 items-center p-5 bg-gray-50 dark:bg-slate-700 rounded-sm outline-none w-full">
                    ...
                    <p className="ml-2 dark:text-white">
                      ...
                      <br /> {getCurrencyNameFromCode(tenant.currency)}
                    </p>
                  </div>
                  <div className="flex gap-4 items-center p-5 bg-gray-50 dark:bg-slate-700 rounded-sm outline-none w-full">
                    <FiWatch className="text-xl text-[#3d50e0]" />
                    <p className="ml-2 dark:text-white">
                      <span className="font-semibold">{t("timezone")}</span>
                      <br /> {tenant.timezone_name}
                    </p>
                  </div>
                </div>
              </>
      ...
      <div
        id="crud-modal"
        ref={addModal}
        className="fixed inset-0 flex hidden items-center justify-center bg-gray-500 dark:border-none0 bg-opacity-75 z-50"
      >
        <div
        ...
        <div className="relative mt-1 w-screen md:w-full max-w-2xl max-h-screen">
          ...
          <div className="relative bg-white dark:bg-[#243040] shadow max-h-[100vh] overflow-auto">
            ...
            <div className="fixed z-50 w-full max-w-2xl flex items-center justify-between p-4 md:p-5 border-b bg-white dark:bg-[#243040] dark:border-[#314051] text-black dark:text-white">
              ...
              <button
                type="button"
                className="text-xl bg-transparent hover:text-slate-500  dark:hover:text-slate-300 outline-none w-8 h-8 ms-auto inline-flex justify-center items-center "
            ...
            <div className="p-4 md:p-5 bg-white dark:bg-[#243040] dark:text-white overflow-y-auto mt-[4.5rem]">
              <div className="flex justify-between items-center">
                <div className="text-xl font-semibold text-[#3d50e0]">
                  {t("general")}
                </div>
                ...
                <div className="flex gap-2 items-center">
                  {modeModal === "Edit" ? (
                    <button
                      className="bg-[#3d50e0] hover:bg-blue-700 text-white font-semibold text-md py-1 px-2 rounded text-nowrap"
                      onClick={updateTenant}
                    >
                      {isLoading ? s("saving") : t("save")}
                    </button>
                  ) : (
              ...
              <div className="grid gap-4 mb-4 grid-cols-2 max-h-screen md:pb-3">
                <div className="col-span-2 md:col-span-1">
                  <label
                    htmlFor="name"
                    className="mb-2 text-sm font-medium  flex"
                  >
                    {t("company-name")}{" "}
                  ...
                  <input
                    ...
                    name="name"
                    id="name"
                    className="bg-gray-50 dark:bg-slate-800 dark:border-none border outline-1 border-gray-300  text-sm rounded-sm outline-none focus:ring-[#3c54d9] focus:border-[#3c54d9] block w-full p-2.5 "
                ...
                <div className="col-span-2 md:col-span-1">
                  ...
                  <input
                    ...
                    name="address"
                    id="address"
                    className="bg-gray-50 dark:bg-slate-800 dark:border-none border border-gray-300  text-sm rounded-sm outline-none focus:ring-[#3c54d9] focus:border-[#3c54d9] block w-full p-2.5 "
                    placeholder={t("holder-address")}
                    required
                    onChange={(e) =>
                ...
                <div className="col-span-2 md:col-span-1">
                  ...
                  <select
                    ...
                    defaultValue={0}
                  >
                    <option disabled value={0} key={0} hidden>
                ...
                <div className="col-span-2">
                  <div className="flex justify-between items-center gap-5">
                    <div className="text-xl w-[50%] font-semibold text-[#3d50e0]">
                      FORMAT
                    </div>
                      ...
                      >
                        ...
                        <div className="flex flex-row gap-3 items-center">
                          <div className="flex items-center gap-1">
                            <input
                              type="radio"
                              id="currencyposition-right"
                              name="currencyposition"
                              value="right"
                              className="hover:cursor-pointer"
                            />
                            <label
                              htmlFor="currencyposition-right"
                              className="hover:cursor-pointer text-xs font-medium"
                            >
                              {t("right")}(-$)
                            </label>
                          </div>
                          ...
                          <div className="flex items-center gap-1">
                            <input
                              type="radio"
                              id="currencyposition-left"
                              name="currencyposition"
                              value="left"
                              className="hover:cursor-pointer"
                            />
                            <label
                              htmlFor="currencyposition-left"
                              className="hover:cursor-pointer text-xs font-medium"
                            >
                              {t("left")}($-)
                            </label>
                ...
                <div className="col-span-2 md:col-span-1">
                  ...
                  <select
                    ...
                    defaultValue={0}
                  >
                    <option value={null} index={0} disabled hidden>
                      Select Language
                    </option>
