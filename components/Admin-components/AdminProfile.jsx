...
import AdminProfileSkeleton from "./Loading/AdminProfileSkeleton";
...
const AdminProfile = () => {
  ...
  const [isLoading, setIsLoading] = useState(false);
    ...
    const getSelectData = async() => {
        await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/timezone`).then((response) => {
            setSelectTimezone(response.data)
        })
        ...
        await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/culture`).then((response) => {
            setSelectCulture(response.data)
        })
  ...
  const validationSchema = yup.object().shape({
    ...
    name: yup.string().required(t("username-error")),
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        formData.append(
          "currency_position",
          values.currency_position === "front" ? true : false
        );
        ...
        if (localStorage.getItem("lang") !== values.language) {
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              router.push(`/${localStorage.getItem("lang")}/admin-dashboard`);
            }
          });
        } else {
        ...
        <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex flex-col items-center">
          <div className="w-full bg-white dark:bg-gray-800 shadow-lg rounded-b-lg overflow-hidden">
            ...
            <form
              ...
              className="p-6 dark:text-gray-200"
            >
              <ProfileSection
              ...
              <ProfileSection
                title={t("edit-international-information")}
                display={roles != "client" ? true : false}
              >
                <ProfileItem
                  icon="culture"
                  label={
                  ...
                  element={
                    <select
                      ...
                      name="culture"
                    >
                      <option value="" disabled hidden hidde>
                        {t("select-culture")}
                      </option>
                  ...
                  element={
                    ...
                    >
                      ...
                      {selectCurrency.map((item, index) => {
                        const [currencyCode, ...currencyNameParts] =
                          item.currency.split("  -  ");
                        return (
                ...
                <ProfileItem
                  icon="position"
                  label={
                    formik.errors.currency_position ? (
                      <label className="text-gray-700 dark:text-gray-300 font-medium mb-2">
                        {t("currency_position")}
                        <span className="text-red-600">*</span>
                      </label>
              ...
              <button
                type="submit"
                className="bg-blue-500 w-full h-12 text-white px-6 py-2 rounded-lg mt-4 hover:bg-blue-600"
              >
                {loading ? (
                  t("saving")
                ) : (
