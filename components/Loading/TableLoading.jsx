...
const TableLoading = () => {
  ...
  return (
    <div className="w-full h-full bg-white rounded-lg dark:bg-slate-700">
      {!usePathname().includes('/dashboard') && (
        <div className="w-32 h-11 rounded-full bg-gray-300 animate-pulse mb-10"></div>
      )}
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-3 overflow-x-auto">
            ...
            <tr>
              {["Column 1", "Column 2", "Column 3", "Column 4", "Column 5", "Column 6"].map((header) => (
                <th key={header} className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300">
                  <span className="bg-gray-300 dark:bg-gray-500 text-nowrap w-20 text-transparent h-10 rounded-full animate-pulse">lorem ipsum</span>
                </th>
              ))}
            </tr>
          </thead>
